...
function AddFoundForm({ dataf, addItemf }) {
	...
	const onSubmit = (e) => {
		...
		e.preventDefault();
		addItemf([ ...dataf, newItem ]);
		toast.success('Your found item has been successfully added!', {
	...
	return (
		<div className="lost-section card shadow-lg p-3 mb-5 bg-white rounded">
			...
			<form className="lost-form" onSubmit={(e) => onSubmit(e)}>
				<div className="form-group">
					...
					<input
						...
						value={title}
						required
						placeholder="Ex: coffee mug"
						onChange={(e) => onChange(e)}
					/>
				</div>
				<div className="form-group">
					<label htmlFor="description">Provide a brief description of the item: </label>
					<input
						id="description"
						name="description"
						value={description}
			...
			{fireRedirect && <Redirect to="/foundItems" />}
		</div>
	);
}
